...
if (name) {
  ...
  if (detail) {
    ...
    setTimeout(() => {
      const detailDataEl = document.querySelector('#detail-data');
      console.log(detailDataEl.offsetHeight);
      image.createImgPreload(
        'image-detail-mobile',
        'block md:hidden w-full h-[273px] mt-5 rounded-[12px] mb-8',
        detail.image
      );
      image.createImgPreload(
        'image-detail',
        `hidden md:block w-1/2 h-[${detailDataEl.offsetHeight}px] rounded-[24px]`,
        detail.image
      );
    ...
    function createExtraList(listData) {
      ...
      listData.forEach((extra, index) => {
        ...
        inputEl.className =
          'appearance-none w-3 h-3 bg-gray-400 checked:bg-primary transition-all rounded-full';
    ...
    function createVariantList(listData) {
      ...
      listData.forEach((variant) => {
        ...
        labelEl.className =
          'border border-gray-400 px-2 md:px-4 py-1 cursor-pointer has-[:checked]:bg-primary has-[:checked]:border-primary has-[:checked]:text-white transition-all rounded-full';
        labelEl.appendChild(inputEl);
        labelEl.appendChild(textEl);
